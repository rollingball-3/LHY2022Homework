...
def trainer(train_loader, valid_loader, model, config, device):
    ...
    for epoch in range(current_episode, n_epochs):
        ...
        for x, y in train_pbar:
            ...
            total_correct += torch.sum(torch.argmax(pred, dim=1) == torch.argmax(y, dim=1))
            total += y.shape[0]
        ...
        writer.add_scalar("Accrate/train", total_correct / total, step)
        ...
        print("Valid acc rate {}".format(total_correct / total))
        writer.add_scalar('Loss/valid', mean_valid_loss, step)
        writer.add_scalar('Accrate/valid', total_correct / total, step)
...
def Rnn_trainer(train_loader, valid_loader, model, config, device):
    # 1、定义损失函数和优化器
    criterion = nn.CrossEntropyLoss(reduction='mean')
    # optimizer = torch.optim.SGD(model.parameters(), lr=config['learning_rate'], momentum=0.9, weight_decay=0.001)
    optimizer = torch.optim.Adam(model.parameters(), lr=config['learning_rate'], weight_decay=0)
